...
void RmScan::next() {
    ...
    while (page_no < file_hdr.num_pages) {
        RmPageHandle page_handle = file_handle_->fetch_page_handle(page_no);
        int next_slot = Bitmap::next_bit(true, page_handle.bitmap, file_hdr.num_records_per_page, slot_no);
